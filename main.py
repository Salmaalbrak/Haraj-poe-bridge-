...
HARAJ_USER_AGENT   = os.getenv("USER_AGENT", "")                  # مهم حسب كلامهم
...
def extract_prefs_from_text(text: str) -> Preferences:
    ...
    # ماركات
    makes = ["تويوتا", "نيسان", "هوندا", "هيونداي", "كيا", "لكزس", "مرسيدس", "بي ام", "bmw", "شفر", "فورد", "مازدا", "جيب", "دودج", "شيري", "جيلي"]
    for mk in makes:
        if mk in t:
    ...
    # مدينة
    cities = ["الرياض", "جدة", "الدمام", "الخبر", "مكة", "المدينة", "بريدة", "الطائف", "أبها", "حائل", "ينبع", "جازان"]
    for c in cities:
        if c in t:
            p.city = c
            break
...
async def gql_search(filters: Dict[str, Any], page=1, limit=10) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        while True:
            ...
            data = r.json()
            if "errors" in data:
                # جرّبوا قالوا مافيه توكن، فنمرر الأخطاء كما هي لو ظهرت
