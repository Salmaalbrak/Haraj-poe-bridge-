...
HARAJ_USER_AGENT  = os.getenv("HARAJ_USER_AGENT", "haraj/5.6.11 (iPhone; iOS 18.1; Scale/3.00)")
HARAJ_ACCESS_TOKEN = os.getenv("HARAJ_ACCESS_TOKEN", "")  # اختياري (غالبًا مو لازم)
...
app = FastAPI(title="Haraj↔Poe Bridge", version="3.1")
...
SESSION_STEP: Dict[str, int] = {}        # مرحلة الحوار لكل محادثة
SESSION_PROFILE: Dict[str, Dict] = {}    # إجابات نمط الحياة/الاهتمامات
...
def format_results_ar(items: List[Dict[str, Any]]) -> str:
    ...
    for it in items[:5]:
        ...
        fuel  = car.get("fuel", "")
        url   = it.get("url", "")
        out.append(f"• {title} — سنة {year} — {price} ريال — {city} — {fuel}/{gear}\n{url}")
    return "\n\n".join(out)
...
async def poe(req: Request):
    ...
    if any(k in low for k in ["reset", "امسح", "امسح الشروط", "ابدأ من جديد", "ابدا من جديد"]):
        SESSIONS[conv_id] = Preferences()
        SESSION_STEP[conv_id] = 0
        SESSION_PROFILE[conv_id] = {}
    ...
    prefs   = SESSIONS[conv_id]
    profile = SESSION_PROFILE[conv_id]

    # بداية محادثة ودّية (زي ChatGPT)
    ...
    # نلتقط أي تفضيلات فنية مذكورة بالنص
    ...
    if step == 7:
        ...
        if m:
            prefs.year_min = int(m.group(1))
            SESSIONS[conv_id] = prefs
        SESSION_STEP[conv_id] = 8  # للبحث

    # تحويل التفضيلات لفلاتر وطلب النتائج
